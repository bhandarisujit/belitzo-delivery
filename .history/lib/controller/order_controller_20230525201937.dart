...
class OrderController extends GetxController implements GetxService {
  ...
  List<OrderModel> _searchOrderList;
  ...
  List<OrderModel> get searchOrderList => _searchOrderList;
  ...
  Future<void> getSearchOrder(String orderId) async {
    _isLoading = true;
    _searchOrderList = [];
    update();
    Response response = await orderRepo.getSearchOrder(orderId);
    if (response.statusCode == 200) {
      response.body.forEach((order) {
        _searchOrderList.add(OrderModel.fromJson(order));
      });

      if (_searchOrderList.isEmpty) {
        showCustomSnackBar('no_order_found'.tr, isError: false);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<bool> acceptOrder(int orderID, int index, OrderModel orderModel,
      bool isFromSearchOrderScreen) async {
    _isLoading = true;
    update();
    ...
    bool _isSuccess;
    if (response.statusCode == 200) {
      if (isFromSearchOrderScreen) {
        _searchOrderList.removeAt(index);
      } else {
